  ...
  return (
    <section aria-label="Trust signals" className="border-y border-border bg-card">
      <div className="mx-auto grid max-w-7xl grid-cols-2 divide-y divide-border sm:grid-cols-4 sm:divide-x sm:divide-y-0">
        <Item value={`${firm.leadAdvocate.yearsOfPractice}+`} label="Years at the Bar" />
        <Item value="3" label="Courts (District · HC · SC)" />
        <Item
          value={
          ...
          label={`${reviews.count} Google reviews`}
        />
        <Item value="AIBE" label="Certified · Bar Council Reg." />
      </div>
    </section>
  );
}
